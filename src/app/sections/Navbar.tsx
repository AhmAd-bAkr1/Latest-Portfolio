...
import { Link } from "react-scroll"; // تأكد من استخدام الأقواس المعقوفة عند الاستيراد

const Navbar = () => {
  const navRef = useRef<HTMLDivElement | null>(null);
  const linksRef = useRef<(HTMLDivElement | null)[]>(new Array(5).fill(null));
  ...
  return (
    <>
      <nav ref={navRef} className="fixed z-50 flex flex-col justify-between w-full h-full px-10 uppercase bg-black text-white/80 py-28 gap-y-10 md:w-1/2 md:left-1/2">
        <div className="flex flex-col text-5xl md:gap-y-2 md:text-6xl lg:text-8xl">
          {["home", "services", "about", "work", "contact"].map((section, index) => (
            <div key={index} ref={setLinkRef(index)}>
              <Link className="transition-all duration-300 cursor-pointer hover:text-white" to={section} smooth offset={0} duration={2000}>
                {section}
              </Link>
            </div>
          ))}
